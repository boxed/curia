...
def strip_p(body):
    ...
    if body.startswith('<p>') and body.endswith('</p>'):
        return body[3:-4].replace('</p>', '').replace('<p>', '<p></p>')
    return body
 
def save_file_for_object(obj, fieldname, data):
    field = getattr(obj, fieldname)
    field.save(data[fieldname].name, data[fieldname])
...
def get_content_type(obj):
    ...
    if isinstance(obj, str):
        ...
        return ContentType.objects.get(name=content_type)
    
    from django.contrib.contenttypes.models import ContentType
    if hasattr(obj, '_meta'):
        return ContentType.objects.get(app_label=obj._meta.app_label, model=obj._meta.module_name)
    else: # assume obj is a class
        return ContentType.objects.get(app_label=obj.app_label, model=obj.module_name)
...
def get_object_or_none(cls, pk):
    if pk == None:
        return None
    else:
        return cls.objects.get(pk=pk)
        
def get_objects_from(cls, **kwargs):
    f = None
    for i in kwargs:
        key = i
        value = kwargs[i]
        ...
        if f == None:
            f = cls.objects.filter(**{key:value})
        else:
            f = f.filter(**{key:value})
    return f
...
def get_current_community():
    return getattr(_thread_locals, 'community', None)
